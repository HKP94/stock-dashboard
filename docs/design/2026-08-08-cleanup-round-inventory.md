...
- 즉 **기능만 머지되고 테스트는 유실된 브랜치**다. 백필의 KR/US 라우팅·종목 단위 격리·
  접미사 폴백이 현재 **무방비**다.
- **후속 실행 결과(2026-08-08, PM 승인 후)**: **12개 중 복원 가능한 것은 0개였다.**
  대상 함수 `_is_kr`·`backfill_prices`·`recompute_indicators`가 전부 사라졌고 모듈이 재설계됐다.
  재판정하니 위 "무방비" 판정은 **과했다**:
    - KR/US 라우팅·5년 창 → `tests/test_benchmark_backfill.py`가 이미 커버(파일·함수 이름이 달라
      내 이름 대조 방식이 못 찾았다. 방법의 한계).
    - 지표·퀀트 재계산 배선 → 같은 파일이 커버.
    - 접미사 폴백 → **설계상 제거**(이제 `market`을 호출부가 넘긴다) — 위험 자체가 없다.
    - ★**종목 단위 격리** → **진짜 미커버**였다(기존 테스트는 1종목 happy path만).
  그래서 옛 테스트를 복원하는 대신 **현행 API 기준으로 격리 가드 5개만 신설**했다
  (`tests/test_backfill_isolation.py`). `backfill.py`는 한 줄도 고치지 않았고 5개 전부 통과 —
  즉 격리 동작 자체는 원래 옳았고, **없던 것은 그 사실을 지켜줄 테스트**였다.
  브랜치는 이제 삭제 가능.
