...
- **포트폴리오 총자산 = 보유종목 평가액 + 현금**(둘 다 KRW 환산). 현금=`portfolio_cash`(통화별), `compute_portfolio`가 `cash_total`/`asset_total`을 snapshot payload에 저장. USD 보유는 `qty×종가(USD)×market_daily.usdkrw`로 환산(환율 없으면 KRW 합계 제외+`fx_missing`). `portfolio.eval_amount`는 원통화, snapshot `total_value`/`asset_total`은 KRW.
- **포트폴리오 스냅샷은 06시·18시 둘 다 재계산(스테일 방지)**: `compute_portfolio`는 `pipeline_analysis`의 **DAILY_STEPS와 REFRESH_STEPS 양쪽**에 있어야 한다. 06시 auto_run은 **KR 장 시작 전**이라 전일 KR 종가로 평가하므로, 18시 news_refresh(KR 장마감 후 당일 종가 반영)에서 **반드시 재계산**해야 총자산이 최신이다. refresh에서 빠지면 총자산이 항상 ~1거래일 스테일로 남는다(KPH 실계좌값 불일치의 근본원인이었음 — 스냅샷이 전일 KR 종가로 고정돼 당일 하락분 미반영). backtest만 refresh에서 제외(무거움). `asof`는 UTC `date.today()`라 06시 KST 실행이 전일 UTC로 라벨될 수 있으나 18시 실행이 당일 UTC+당일 종가로 갱신·supersede.
