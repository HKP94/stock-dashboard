...
## 현행 상태 스냅샷 (2026-08-08 기준 — 정리 라운드 S4 동기화)

> 이 문서 위쪽 STEP 0~4는 **초기 구축 런북**이라 지금은 대부분 완료 상태다.
> 지금 합류하는 사람이 알아야 할 "현재"는 아래와 같다.

**유니버스**: 활성 57종목(KR 27 · US 30). 보유 4종(001450.KS·005930.KS·085670.KQ·MSFT).

**최근 확정된 계약** — 상세는 `CLAUDE.md` §9(데이터 계약)·§10(스케줄 책임 경계):
- KR 티커 접미사 = 실제 상장시장(`.KS`/`.KQ`). 레거시 6건 정정 완료(s1 마이그레이션).
- 수급 소스 = KB IVU10430(主) → pykrx(폴백). **KB는 백만원 단위**, 외국인 = `fgnr + ntv_fgnr`.
- KRX 수급 확정 시각 = **18:05 KST**(16·17시는 잠정치).
- USD/KRW 정본 = `db.latest_usdkrw`(최신 non-null).
- 실적: `earnings_calendar`(일정·EPS 결과)와 `fundamentals`(재무제표)는 **분리** — 후자는 소스 지연이 있다.

**자동화 설치(집 PC 필수)**: `bash scripts/install_local_automation.sh` — launchd 4잡
(18:30 수급 / 22:30·08:00 export / 대시보드 상주 / 주간 발굴). 전제조건은 `README.md` §1-b.

**KB Open API 상태**: 시세·수급 조회는 정상(CI 호출도 실증). **계좌 연동은 보류** —
실투자 계좌를 앱키에 연결할 수 없어(오픈베타 제약) 잔고 자동화(`feat/kb-portfolio-sync`)는
브랜치로 보존 중. 계좌 연결이 풀리면 dry-run부터 재개한다.
