...
# ── 상수 ─────────────────────────────────────────────────────
# Piotroski 사전필터 임계(이하이면 탈락 → fallback 표식).
# ★2를 쓰는 이유 = **척도 보정**(PM 결정 2026-07-22). Piotroski 관례값 3은 **9점 척도** 기준인데
# 이 구현은 signal 7(주식발행)·8(매출총이익률)을 수집하지 않아 **실질 만점이 7**이다
# (piotroski_fscore 주석·claude.md §스크리너 참조). 9점 임계를 7점 척도에 그대로 적용하면
# 전 종목이 최대 2점 손해 보고 과도 제외된다(실측: 탈락 20종목 중 17건이 이 경로).
# 7/9 비례 환산 시 3×7/9≈2.3 → 정수 임계 2가 원 의도에 가장 가깝다.
# signal 7·8을 실제로 수집하게 되면 만점이 9로 돌아가므로 임계도 3으로 되돌려야 한다.
FSCORE_FILTER_THRESHOLD: int = 2
