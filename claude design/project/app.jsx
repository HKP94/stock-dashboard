...
function App() {
  ...
  const [ticker, setTicker] = React.useState(D.stocks[0].t);
  const [newsFilter, setNewsFilter] = React.useState(null);
  ...
  return <div style={{ minHeight: "100vh", paddingBottom: 52 }}>
    ...
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: "20px 24px 32px" }}>
      {tab === "overview" && <Overview nav={nav} goNews={goNews} />}
      {tab === "detail" && <StockDetail ticker={ticker} nav={nav} />}
      {tab === "news" && <NewsTab filterTicker={newsFilter} setFilterTicker={setNewsFilter} nav={nav} />}
      {tab === "screener" && <Screener nav={nav} />}
      {tab === "market" && <Market />}
      {tab === "research" && <Research nav={nav} />}
    </div>
    ...
    <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 40, background: "rgba(255,255,255,0.92)", backdropFilter: "blur(6px)", borderTop: `1px solid ${C.line2}` }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "9px 24px", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{ fontSize: 11, color: C.ink3, lineHeight: 1.4 }}>표시 신호는 근거·신뢰도를 포함하며 자동 주문을 실행하지 않습니다.</span>
      </div>
    </div>
  </div>;
}
