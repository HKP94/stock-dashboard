...
- [ ] **신규-D 종목 액션 재정의(설계 완료, 구현 대기)** (2026-06-25): 5-A의 비중 권고(목표 비중·늘림/줄임)가 집중 투자 철학과 충돌(효성중공업 36%를 "줄여라"로 판단). 종목 카드를 **"비중 판단"→"보유성격(장기보유/모멘텀/단기) 판단 + 집중 리스크 관찰"**로 재정의. 비중 권고는 카드 **표시에서만 제거**(target_weight/weight_action/direction 컬럼·계산은 보존, 5-C 포트폴리오 탭 재사용). 보유성격은 기존 재료(수동분해 단/중/장 + 퀀트·모멘텀·정배열·안전마진 + SMA 진입구간)로 **결정론 규칙(A)**, LLM은 해설만. 집중 리스크는 비중·(신규-A 예정)베타를 "관찰"로만 서술(강요·"줄여라" 금지, 톤 가이드·금지어 가드). 진입/이탈·신뢰도·divergence·3축 비합산은 유지. 설계: `docs/superpowers/specs/2026-06-25-newd-stock-character-redesign.md`.
