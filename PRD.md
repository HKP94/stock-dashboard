...
- [ ] **신규-A1 시장 상관·베타(설계 완료, 구현 대기)** (2026-06-25): 종목과 자국 지수(KR→^KS11, US→^GSPC)의 **베타·상관**을 `prices_daily`+`index_daily`(저장·5년, 라이브 yfinance 아님)로 **결정론 계산**(252거래일·env 덮어쓰기, OLS cov/var, 결측·상장부족 시 None). `quant_scores`에 `beta`/`market_corr` **ADD COLUMN**(신규 테이블 아님, 종목별 최신 조회 유지). 퀀트 축 안의 **별도 시장 민감도 팩터**로만 — **composite 합산 금지**(3축 비합산). 신규-D 집중 관찰 노트의 빈 베타 자리 자동 충전(사실+영향, 평가어 금지). 종목상세 퀀트 축에 "시장 민감도(베타·상관+벤치마크·기간)" 보조 표시. §F7 진짜 계산(룩어헤드 없음, 회고 아님). 분석 파이프라인(`compute_quant`) 소관. 시장방향 연동(고베타 매력도 하향)은 5-B 이후. 설계 `docs/superpowers/specs/2026-06-25-newa1-market-beta-design.md`.
