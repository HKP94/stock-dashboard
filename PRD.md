...
- [ ] **신규-A2 매력도 3축 종합 등급(설계 완료, 구현 대기)** (2026-06-26): 매력도 3축(퀀트·컨센서스·내 판단)을 하나의 통합 카드에 묶어 ① 매매 기준이 되는 **명확한 등급**(매수/관망/축소, 결정론·★핵심) ② 등급을 받치는 **3축 종합 해설**(LLM) ③ **3축 괴리 식별**(결정론)을 보여주는 설계. **단일 통합점수 금지** — 등급은 축의 방향(강/중/약) **정렬 패턴**(방향 투표, 값 합산 아님)에서 규칙 테이블로 도출. 임계는 기존 `AxesCard._level`(퀀트 60/40·컨센서스 20/5·별점 4/2) 재사용. 시장 점수(5-B)·베타(A1)는 **퀀트 축 경로로만**(약세장+고베타 시 퀀트 축 한 단계 보수화, 강세장 미상향·비대칭; 컨센서스·내 판단 불변·composite 미합산). 저장은 `stock_action_advice` 컬럼 확장(`grade/grade_confidence/grade_basis` ADD COLUMN, 신규 테이블 없음). LLM은 등급 근거 한 단락만(숫자·등급 생성 금지, 폴백 결정론 템플릿). 발굴=스크리너 등급 정렬·필터("지금 매수 등급"). §F7 진짜 계산. 등급=종합 액션 제언 단계 편입. 설계 `docs/superpowers/specs/2026-06-26-newa2-axis-grade-design.md`. 구현은 승인 후 별도. 코드 변경 없음.
