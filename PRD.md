...
- *v8.5 (2026-07-06) 포트폴리오 총자산 스테일 수정(KPH 계좌값 불일치): [진단] 종목별 [qty×최신가×FX]+현금 decimal 손계산 vs 대시보드 스냅샷 대조 — 계산식·FX(usdkrw 1529, 히스토리상 글리치 아님)·현금·cost 전부 정합, 그러나 스냅샷(asof 07-05) KRW eval이 **07-03 KR 종가와 정확 재현**(오늘 07-06 종가는 하락: 298040 3.32M→3.12M·010120 229.5k→218.5k). [근본원인] `compute_portfolio`가 `DAILY_STEPS`(06시)엔 있으나 **`REFRESH_STEPS`(18시)엔 빠짐**. 06시 auto_run은 KR 장 시작 전이라 전일 종가로 평가하고, 18시 news_refresh가 당일 KR 종가를 넣는데 포트폴리오를 재계산 안 해 **총자산이 항상 ~1거래일 스테일**(당일 하락 미반영→과대평가). [수정] `REFRESH_STEPS`+refresh 브랜치에 `_step_portfolio` 추가(backtest만 제외, 경량·결정론). [검증] decimal 재계산 스냅샷 asset_total 9,735,972.82 = 손계산 9,735,972.82(**diff 0.00**), 스테일 10,082,512→최신 9,735,972(KR 하락분 반영). 계산 로직·회귀 없음(값 갱신만). 단위테스트 갱신(refresh가 포트폴리오 재계산·backtest 제외, pytest 693). 영속화=다음 18시 cron. CLAUDE.md 포트폴리오 규칙 갱신 — Claude Code.*
