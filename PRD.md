...
- [ ] **Wave 5-B 시장 방향·매력도 점수(설계 완료, 구현 대기)** (2026-06-25): "지금 투자 적합한 환경인가·어느 방향인가"를 **결정론 점수(0~100)+방향(강세/중립/약세)+신뢰도**로 평가. 입력=지수 추세/모멘텀(`index_daily`)·변동성(VIX)·매크로(`macro_indicators` 금리·CPI·DXY·WTI)·시장폭(정배열율); 뉴스 심리는 LLM 해설 전용(점수 코어 오염 방지). 정확도 가드: 컴포넌트 정합성·divergence 점검, 단일 지표 의존 회피, 엇갈리면 신뢰도 하향(가짜 확신 금지). 저장은 신규 `market_score(asof, region, score, direction, confidence, components)` 시장 단위 이력. 종목 영향은 **신규-A1 베타 경로로만** — 이번은 종목 카드 "시장조정 관찰"(시장점수+베타→조건부 영향 사실 서술) **표시까지**, **composite 미합산 유지**(실제 반영은 보수적 후속). LLM은 방향 해설만(매매 단정 금지). §F7: 지수=진짜 계산, 매크로=발표 시점만(룩어헤드 금지). 점수=분석(`pipeline_analysis`)·해설=종합(`pipeline_synthesis`). 트레이딩(수급·차트)은 신규-E 별도. 설계 `docs/superpowers/specs/2026-06-25-wave5b-market-direction-design.md`.
