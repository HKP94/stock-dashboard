...
- [x] **pykrx/KRX 호출 무한 대기**: `ingest_kr._pykrx_ohlcv`의 KRX OHLCV 호출에 timeout 부재로 KRX 무응답 시 영원히 매달려 06시 split이 90분 워크플로 timeout으로 잘림(#37에서 Gemini·yfinance엔 걸었으나 pykrx 누락). `external_timeout.run_with_timeout`로 하드 타임아웃(`KRX_HTTP_TIMEOUT_S` 기본 40초, env 덮어쓰기) 적용 → 무응답 종목은 `ExternalCallTimeout`으로 끊고 종목 단위 격리(skip+`runs.errors`)로 다음 종목 진행. pykrx 로그인 안내(`KRX_ID/PW` 미설정)는 익명 조회가 동작하므로 무해 — stdout 스피너 소음만 억제(시크릿 미저장). fetch_kr_prices/backfill/refresh 전 경로 커버. 단위 테스트 3개.
