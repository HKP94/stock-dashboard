...
### F1 잔고 자동화 (KB증권 Open API, 로컬 전용)
- [x] **1단계 잔고 조회 자동화** (2026-08-06): `src/kb_client.py`(토큰 발급·캐시·401 재발급·지수백오프·**조회 API 화이트리스트 가드**) + `src/collect_kb_portfolio.py`(SSQM2952 국내잔고 + SPQM2226 해외잔고·외화예수금 + SSQM0004 예수금 → `portfolio_holdings`/`portfolio_cash` **전량 스왑**, `--dry-run`). `scripts/local_refresh.py`의 `compute_portfolio` 앞단에 배선(22:30·08:00 공통). CI 미배선(키는 로컬 `.env`만). 주문·정정·취소 API 미구현 + 회귀 테스트. 단위테스트 58.
- [ ] **실계좌 검증**: KB 앱키에 연결된 계좌가 보유내역 없는 계좌 → 잔고 3종이 모두 0건 응답. KPH가 KB 개발자포털에서 실투자 계좌 연결 후 `--dry-run` 재검증 필요.
- [ ] **2단계**: 수급 파일럿(투자자·거래원 등) 및 CI 배선 가능성 검토.
...
---
*변경 이력:*
- *v6.9 (2026-08-06) F1 잔고 자동화 1단계(KB증권 Open API) 구현: 수동 입력하던 `portfolio_holdings`/`portfolio_cash`를 실계좌 조회로 대체. `src/kb_client.py` — /oauth2/token(grantType=client_credentials) 발급·메모리 캐시·만료/401 재발급, POST 헬퍼(Authorization: bearer), 통신오류·5xx 지수백오프 3회(무한재시도 금지), 키·토큰 마스킹(`_scrub` — 로그·예외 문자열 노출 금지), **조회 API 화이트리스트 가드**(ssqm2952·spqm2226·ssqm0004·siqm4900만 허용, SSAM/SKAM/SPAO 주문 계열은 `KBOrderAPIBlocked`). ★명세 엑셀 미기재 사항 실호출로 확인 — 전 API가 `{"dataHeader":{...},"dataBody":{...}}` 봉투(평면 body는 500 E021), 조회 API는 dataHeader에 ipAddr·macAddr 필수, 응답 processFlag A=정상(0건 포함)/B=오류. `src/collect_kb_portfolio.py` — 3콜(SSQM2952 국내잔고 Record1 + SPQM2226 해외잔고 Record2·통화별 예수금 Record1 + SSQM0004 예수금)로 **전량 스왑**(KB에 없는 보유행 제거=매도 반영). 티커 매핑=watchlist·기존보유 우선 → `is_dtl_typ_cd`(11=코스피 .KS / 12=코스닥 .KQ, 잔고 응답에 이미 포함되어 추가 호출 0) → 미해결은 보류 리포트. 소수점 보유(`hld_q_p6`)는 정수분과 합산하되 평가금액÷현재가로 교차검증(이중계상 방지). 현금은 KRW(예수금 D+2)·USD(해외 외화예수금) 분리. **안전장치**: 보유 0건이면 삭제·upsert 전면 스킵+WARNING(API 오류 시 전멸 방지), 미해결 종목 존재 시 삭제만 스킵. `--dry-run`(DB 무변경, 현행 대비 diff 출력). 배선=`scripts/local_refresh.py` compute_portfolio 앞단(22:30·08:00 공통), **CI 미배선**(키를 GitHub에 올리지 않음). 단위테스트 58(주문가드 30·토큰 재발급/만료·마스킹·매핑·전량스왑 안전장치, 전부 모킹) — 820 passed. ⚠️ 실계좌 검증 보류: 현 앱키에 연결된 계좌가 보유내역 없는 계좌라 잔고 3종 모두 0건(시세 API는 정상 응답) → KPH의 계좌 연결 확인 후 재검증 — Claude Code.*
